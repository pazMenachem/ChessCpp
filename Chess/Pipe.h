...
public:
	...
	bool connect()
	{
		...
		printf(_T("The named pipe, %s, is connected.\n"), strPipeName);
		return true;
	...
	bool sendMessageToGraphics(char* msg)
	{
		...
		{
			printf(_T("WriteFile failed w/err 0x%08lx\n"), GetLastError());
			return false;
		}

		printf(_T("Sends %ld bytes; Message: \"%s\"\n"),
	...
	std::string getMessageFromGraphics()
	{
		...
		{
			printf(_T("ReadFile failed w/err 0x%08lx\n"), GetLastError());
			return "";
		}

		printf(_T("Receives %ld bytes; Message: \"%s\"\n"),
			cbBytesRead, chReply);
